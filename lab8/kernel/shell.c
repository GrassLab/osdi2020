...
void fat32_test() {
  ...
  const char *write1 = "Hello";
  a->f_pos = 0;
  vfs_write(a, write1, 5);
  ...
  vfs_read(a, buf, 5);
  buf[5] = '\0';
  uart_println("[fat] file: %s, text: %s", filename, buf);

  const char *write2 = "Ni Hao";
  a->f_pos = 0;
  vfs_write(a, write2, 7);
...
void shell() {
  ...
  while (1) {
    ...
    SWITCH_CONTINUE(buf, "fat", fat32_test);
