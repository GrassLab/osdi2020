...
int register_filesystem(struct filesystem* fs) {
  ...
  if(my_strcmp(fs->name, "tmpfs", 4) == 0)
  {
    fs->setup_mount = tmpfs_mount_setup;
    return 1;
  }
  else if(my_strcmp(fs->name, "fat32", 5) == 0)
  {
...
int found_last_dir_vnode(const char* pathname, struct vnode **found)
{
  ...
  while(1)
  {
    ...
    //my_printf("start: %d, end : %d\r\n", pstart, pend);
    if(pend != 0)
    {
      for(int i=0; i < vnow->num_of_child; i++)
      {
        //my_printf("%s %s %d\r\n",vnow->child[i]->name, (pathname+pstart), (pend-pstart));
