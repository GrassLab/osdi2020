...
void print_info()
{
	...
	serial_res &&printk("Serial num: %x\n", serial_num);
...
void __attribute__((section(".bootloader"))) load_kernel()
{
	printk("start load kernel\n");
	...
	size |= uart_getc() << 24;
	printk("Image size: %d\n", size);
	uint32_t *load_base = (uint32_t *)address;
	while (size--) {
		*load_base++ = uart_getc();
	} // load kernel image
	// printk("Ready to jump\n");
	asm volatile("mov x30, %0" ::"r"(address) :);
}
