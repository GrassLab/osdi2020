...
// send kernel to rpi
void send_kernel(int fd, const char *file, uint32_t startAddr) {
    int file_fd;
    off_t off;
    uint32_t size;
    ssize_t pos;
				...
				if (p == q) {
				    ...
				    if (breaks == 3) {
						...
						if(argc == 4){
							...
							int inputAddrlen = strlen(argv[3]);
							char **ptr;
							long addr = strtol(argv[3], ptr, 16);
							printf("[load address to %s]\n", argv[3]);
							startAddr = addr;
						}else{
							printf("[load to default address 0x80000]\n");
						}
						send_kernel(fd, argv[2], startAddr);
						breaks = 0;
				    }
				} else {
				    while (breaks > 0) {
