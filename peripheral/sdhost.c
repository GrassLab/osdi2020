...
static int
sd_cmd (unsigned cmd, unsigned int arg)
{
  set (SDHOST_ARG, arg);
...
void
readblock (int block_idx, void *buf)
{
...
void
writeblock (int block_idx, void *buf)
{
  unsigned int *buf_u = (unsigned int *) buf;
  int succ = 0;
  if (!is_hcs)
